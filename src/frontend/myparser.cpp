...
static ast::StmtList* p_StmtList(){
  ...
  while(!isFollow[SymbolType::StmtList][next_token.type]){
    if(isFirst[SymbolType::Statement][next_token.type]){
      ...
      // Complete the action if the next is a declaration. Remeber to consume ';'.
...
static ast::Statement* p_Statement(){
  ...
  if(isFirst[SymbolType::Expression][next_token.type]){
    ...
    if(next_token.type == TokenType::RETURN){
      ...
      lookahead(TokenType::SEMICOLON);
      return new ast::ExprStmt(nullptr, next_token.loc);
    } else {
...
static ast::IfStmt* p_If(){
   ...
   **/
    ...
    ast::Expr* expr = p_Expression();
    lookahead(TokenType::RPAREN);
    ast::Statement* tstmt = p_Statement();
    ast::Statement* fstmt;
    if (next_token.type == TokenType::ELSE) {
        lookahead(TokenType::ELSE);
        fstmt = p_Statement();
    } else {
        fstmt = new ast::EmptyStmt(next_token.loc);
    }
    return new ast::IfStmt(expr, tstmt, fstmt, token.loc);
}
...
static ast::Expr* p_Assignment(){
   ...
   **/
    ...
    ast::Lvalue* lvalue = dynamic_cast<ast::LvalueExpr*>(node)-> lvalue;
    ast::VarRef* var = dynamic_cast<ast::VarRef*>(lvalue);
    node = new ast::AssignExpr(var, rhs, node->getLocation());
  }
  return node;
}
...
static ast::Expr* p_LogicalAnd(){
  ...
  while(next_token.type == TokenType::AND){
    ...
    ast::Expr* op2 = p_Equality();
    node = new ast::AndExpr(node, op2, token.loc);
  }
...
static ast::Expr* p_Relational(){
  ...
  while(next_token.type == TokenType::LT || next_token.type == TokenType::GT ||
    ...
    Token operation = lookahead();
    ast::Expr* op2 = p_Additive(); 
 
    switch(operation.type){
      case TokenType::LT:
        node = new ast::LesExpr(node, op2, operation.loc);
        break;
      case TokenType::GT:
        node = new ast::GrtExpr(node, op2, operation.loc);
        break;
      case TokenType::LEQ:
        node = new ast::LeqExpr(node, op2, operation.loc);
        break;
      case TokenType::GEQ:
        node = new ast::GeqExpr(node, op2, operation.loc);
        break;
      default:
        break;
    }
